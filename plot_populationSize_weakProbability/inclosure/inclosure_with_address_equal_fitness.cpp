...
std::vector<int> Inclosure::run(){
    ...
    if (test_problem_debug){
        ...
        buffer[0].set_fitness(9);
        fitness[0] = 9;
        buffer[enum_size-2].set_fitness(8);
        fitness[enum_size-2] = 8;

        buffer[enum_size-8].set_fitness(7);
        fitness[enum_size-8] = 7;
        buffer[enum_size-7].set_fitness(7);
        fitness[enum_size-7] = 7;
...
//with address equal fitness version
    for (int u=1; u<ell; u++) {
        ...
        }while (fitness[buffer[w-1].getGene(0)] == fitness[buffer[w].getGene(0)]);
...
// without address equal fitness version
//    for (int u=1; u<ell; u++) {
//         int val = 1 - buffer[b].getVal(u); // flip bit
            ...
            if(not_find_smaller_epi)
            {
                ...
                { 
                    ...
                    do
                    {
                        ...
                        if(first == 1)
                        {
                            my_fitness = fitness[buffer[chrom_index].getGene(0)];
                        }  
                        ...
                        if(fitness[buffer[chrom_index].getGene(0)] == my_fitness)
                        {
                        ...
                        if(chrom_index <= std::pow(2, ell)-2) chrom_index++;

                    }while(fitness[buffer[chrom_index-1].getGene(0)] == fitness[buffer[chrom_index].getGene(0)]);
